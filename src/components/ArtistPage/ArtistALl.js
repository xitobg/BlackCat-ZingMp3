   ...
   return (
      <div>
         <div className="">
                  ...
                  <div className="main_topchart mt-2 w-full">
                     <div className="container_zing-chart">
                        <div className="max-h-[260px] overflow-y-auto zing-chart_list pt-2">
                            {dataSelector?.items && dataSelector?.items.length > 0 && dataSelector?.items.map((e, index) => {
            ...
            {dataSelector2 && dataSelector2.map((e) => {
                     ...
                     return (
                        <PlayListSelector classAdd="artist-mv" key={uuidv4()} title={e.title}>
                           {e?.items?.map((item, index) => {
                              if(index > 2) return
                              return <MvItem key={uuidv4()} data={item} isAritst/>
                           })}
                        </PlayListSelector>
                     );
                  };
